...
export function NewsGrid({ initialNews, initialTotalPages, search }: NewsGridProps) {
	...
	if (initialNews.length === 0) {
		return (
			<div className="my-16 rounded-lg bg-white p-10 text-center shadow-md">
				...
				<h4 className="mb-2 font-serif text-xl font-bold text-gray-800">Nenhuma notícia encontrada</h4>
				<p className="mb-6 text-gray-600">
					{search
						? "Não foram encontradas notícias que correspondam à sua pesquisa."
						: "Não existem notícias publicadas no momento."}
				</p>
				{search && (
					<Link
						href="/noticias"
			...
			loader={
				<div className="my-8 flex justify-center">
					<div className="flex items-center gap-3">
						...
						<span className="text-amber-800">Carregando mais notícias...</span>
					</div>
				</div>
			}
			endMessage={
				news.length >= 12 ? (
					<div className="my-8 text-center text-gray-600">
						<p>Você chegou ao final das notícias disponíveis.</p>
					</div>
				) : undefined
			}
		>
			<div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
				{news.map((item: News, i) => (
					<div
						key={item.id}
