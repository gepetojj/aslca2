...
export function ShareButton({ title, url }: ShareButtonProps) {
	...
	return (
		<button
			onClick={handleShare}
			className="flex items-center gap-2 rounded bg-amber-700 px-4 py-2 text-sm font-medium text-white hover:bg-amber-800"
			aria-label={!!navigator.share ? "Compartilhar notícia" : "Copiar link da notícia"}
		>
			{copied ? (
				<>
					<IconCheck size={16} />
